...
GIFT_VOUCHER_EVENT_TYPES = (
    ("regular_session", "Class"),
    ("private", "Private Lesson"),
    ("workshop", "Workshop"),
)
...
class Event(models.Model):
    ...
    slug = AutoSlugField(populate_from=["name", "date"], max_length=40, unique=True)
    ...
    def spaces_left(self):
        booked_number = (
            ...
            .filter(event__id=self.id, status="OPEN", no_show=False)
            .count()
        )
    ...
    def can_cancel(self):
        ...
        if now.month in [3, 10]:
            ...
            dst_diff_in_hrs = (
                now_local.dst().seconds - event_date_local.dst().seconds
            ) / (60 * 60)
        ...
        return self.allow_booking_cancellation and (
            time_until_event > cancellation_period
        )
...
class BaseVoucher(models.Model):
    ...
    name = models.CharField(
        null=True, blank=True, max_length=255, help_text="Name of recipient"
    )
    message = models.TextField(
        null=True, blank=True, max_length=500, help_text="Message (max 500 characters)"
    )
    ...
    def clean(self):
        if not (self.discount or self.discount_amount):
            raise ValidationError(
                "One of discount (%) or discount amount (fixed £ amount) is required"
            )
        if self.discount and self.discount_amount:
            raise ValidationError(
                "Only one of discount (%) or discount amount (fixed £ amount) may be specified (not both)"
            )
...
class ItemVoucher(BaseVoucher):
    ...
    def paid_and_unpaid_items(self, user=None):
        ...
        if user is not None:
            return {k: v.filter(user=user) for k, v in all_items.items()}
        return all_items
    ...
    def used_items(self, user=None):
        ...
        if user is not None:
            return {k: v.filter(user=user) for k, v in used_items.items()}
        return used_items
...
class GiftVoucherType(models.Model):
    ...
    event_type = models.CharField(
        ...
        null=True,
        blank=True,
        choices=GIFT_VOUCHER_EVENT_TYPES,
        unique=True,
    ...
    active = models.BooleanField(
        default=True,
        help_text="Display on site; set to False instead of deleting unused gift voucher types",
    )
    duration = models.PositiveIntegerField(
        default=6, help_text="How many months will this gift voucher last?"
    )
    ...
    def clean(self):
        ...
        if valid_for_count == 0:
            raise ValidationError(
                "One of event type, membership, or a fixed voucher value is required"
            )
        else:
            raise ValidationError(
                "Select ONLY ONE of event type, membership or a fixed voucher value"
            )
        ...
        if self.voucher and not self.voucher.activated:
            ...
            if self.gift_voucher_type.duration:
                self.voucher.expiry_date = end_of_day_in_utc(
                    ...
                    + relativedelta(months=self.gift_voucher_type.duration)
                )
            self.voucher.save()
    ...
    def save(self, *args, **kwargs):
        ...
        if self.item_voucher is not None:
            if self.gift_voucher_type.membership_type:
                changed = list(self.item_voucher.membership_types.all()) != [
                    self.gift_voucher_type.membership_type
                ]
            else:
                changed = self.item_voucher.event_types != [
                    self.gift_voucher_type.event_type
                ]
        if self.total_voucher is not None:
            changed = (
                self.total_voucher.discount_amount
                != self.gift_voucher_type.discount_amount
            )
        ...
        if (new and not self.voucher) or changed:
            ...
            if self.gift_voucher_type.discount_amount:
                ...
                if self.gift_voucher_type.membership_type:
                    ...
                    self.item_voucher.membership_types.add(
                        self.gift_voucher_type.membership_type
                    )
                else:
...
class Membership(models.Model):
    ...
    membership_type = models.ForeignKey(
        MembershipType, related_name="memberships", on_delete=models.CASCADE
    )
    paid = models.BooleanField(default=False)
...
class Booking(models.Model):
    ...
    no_show = models.BooleanField(
        default=False,
        help_text="Booked but did not attend OR cancelled "
    ...
    def booked_with_membership_within_allowed_time(self):
        ...
        if self.membership:
            return (self.date_rebooked and self.date_rebooked > allowed_datetime) or (
                self.date_booked > allowed_datetime
            )
        return False
    ...
    def cleanup_expired_bookings(cls, user=None, use_cache=False):
        ...
        queries = Q(
            event__date__gt=timezone.now(), status="OPEN", no_show=False, paid=False
        ) & (
            Q(
                checkout_time__lt=timezone.now()
                - timedelta(seconds=checkout_buffer_seconds)
            )
    ...
    def _is_rebooking(self):
        if not self.pk:
            return False
        was_cancelled = (
            self._old_booking().status == "CANCELLED" and self.status == "OPEN"
        )
    ...
    def clean(self):
        ...
        if self.attended and self.no_show:
            raise ValidationError(_("Cannot mark booking as both attended and no-show"))
    ...
    def save(self, *args, **kwargs):
        ...
        if cancellation:
            ...
            if (
                not self.event.cancelled
                and self.event.cancellation_fee > 0
                and not self.event.can_cancel()
            ):
...
def outstanding_fees_total(self):
    bookings_with_fees = self.bookings.filter(
