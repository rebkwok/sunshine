...
var processMembershipPurchaseRequest = function()  {
    ...
    var month_str = $button_just_clicked_on.data('month_str');
    var year = $button_just_clicked_on.data('year');
    var warn_for_current = $button_just_clicked_on.data('warn_for_current');
    ...
    if (warn_for_current) {
      $jq('#confirm-dialog').dialog({
        ...
        title: "Warning!",
        open: function() {
          const contentText = "You are buying a membership for <strong>THIS MONTH</strong>.  It can only be used for classes in <strong>" + month_str + "</strong>."
          $jq(this).html(contentText + "<br><br>Please confirm you want to continue.");
        },
        buttons: [
            {
                text: "Continue",
