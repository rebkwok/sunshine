...
class EventAdmin(DjangoObjectActions, admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ("Contacts", {"fields": ("contact_email", "email_studio_when_booked")}),
        ("Payment Information", {"fields": ("cost",)}),
        (
    ...
    def cancel_event(self, request, queryset):
        for obj in queryset:
            ...
            event_type = (
                "class"
                if obj.event_type == "regular_session"
                else event_names[obj.event_type]
            )
...
class GiftVoucherTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        (
            "Voucher type",
            {
                "fields": ("membership_type", "event_type", "discount_amount"),
...
class GiftVoucherAdmin(DjangoObjectActions, admin.ModelAdmin):
    ...
    def link(self, obj):
        return mark_safe(f'<a href="{obj.get_voucher_url()}">{obj.voucher.code}</a>')
