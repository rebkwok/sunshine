...
class BaseEventListView(ListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        email_waiting_lists(
            event_ids_from_expired_bookings, host=host_from_request(request)
        )
        ...
        if self.event_day in DAYS.keys() and self.event_time:
            ...
            try:
                ...
                event_ids = [
                    event.id
                    for event in events
                    if event.date.weekday() == weekday
                    and event.date.minute == min
                    and event.date.hour
                    == hour - (event.date.astimezone(localtz).dst().seconds / 3600)
                ]
    ...
    def _get_location_data(self):
        ...
        if len(active_locations) == 1:
            default_tab = next(
                k for k, v in venue_locations if v == active_locations[0]
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["workshops_available_to_book"] = alternative_events.filter(
            event_type="workshop"
        ).exists()
        context["classes_available_to_book"] = alternative_events.filter(
            event_type="regular_session"
        ).exists()
        context["privates_available_to_book"] = alternative_events.filter(
            event_type="private"
        ).exists()
        ...
        form = EventsFilter(
            event_type=self.event_type, initial={"name": self.event_name}
        )
            ...
            location_events.append(
                {
                    ...
                    "paginator_range": paginated_queryset.paginator.get_elided_page_range(
                        paginated_queryset.number
                    ),
                }
            )
