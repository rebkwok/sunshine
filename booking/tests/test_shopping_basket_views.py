...
class ShoppingBasketMixin:
    def setUp(self):
        ...
        self.membership_type = baker.make(
            MembershipType, name="4 per month", number_of_classes=4, cost=20
        )
        future = timezone.now() + timedelta(1)
        past = timezone.now() - timedelta(1)
        self.regular_class = baker.make(
            Event, event_type="regular_session", cost=10, date=future
        )
        self.past_class = baker.make(
            Event, event_type="regular_session", cost=10, date=past
        )
...
class ShoppingBasketViewTests(ShoppingBasketMixin, TestCase):
    ...
    def test_with_unpaid_bookings(self):
        booking = baker.make(
            Booking, event=self.regular_class, user=self.user, paid=False
        )
    ...
    def test_with_unpaid_items(self):
        ...
        assert sorted(
            resp.context_data["unpaid_booking_info"], key=lambda x: x["original_cost"]
        ) == [
            {
                "booking": booking1,
                "original_cost": 10,
    ...
    def test_voucher_application(self):
        ...
        resp = self.client.post(
            self.url, data={"add_voucher_code": "add_voucher_code", "code": "test"}
        )
        assert list(resp.context_data["unpaid_membership_info"]) == [
            {
                "membership": membership,
    ...
    def test_voucher_application_fixed_amount(self):
        ...
        assert sorted(
        ...
        ) == [
            {
                "booking": booking1,
                "original_cost": 10,
                "voucher_applied": {"code": "test", "discounted_cost": 5},
            },
            {
                "booking": booking2,
                "original_cost": 20,
            ...
            },
            {
                "booking": booking3,
                "original_cost": 30,
                "voucher_applied": {"code": "test", "discounted_cost": 25},
            },
        ]
        assert list(resp.context_data["applied_voucher_codes_and_discount"]) == [
            ("test", None, 5)
    ...
    def test_remove_voucher(self):
        ...
        resp = self.client.post(
            self.url,
            data={"remove_voucher_code": "remove_voucher_code", "code": "test"},
        )
        assert resp.context_data["total_cost"] == 20
        membership.refresh_from_db()
        assert membership.voucher is None
    ...
    def test_existing_voucher_removed_from_booking_if_invalid(self):
        voucher = baker.make(
            ItemVoucher, code="test", discount=50, event_types=["regular_session"]
        )
        # booking for different event type
        event = baker.make_recipe("booking.future_PV", cost=15)
        booking = baker.make_recipe(
            "booking.booking", event=event, user=self.user, voucher=voucher
        )
        resp = self.client.get(self.url)
    ...
    def test_refresh_voucher(self):
        ...
        voucher.save()
        resp = self.client.post(
            self.url, {"code": "test", "refresh_voucher_code": True}
        )
        assert resp.context_data["total_cost"] == 20  # discount not applied
        membership.refresh_from_db()
        assert membership.voucher is None
    ...
    def test_voucher_validation(self):
        ...
        # invalid code
        resp = self.client.post(
            self.url, data={"add_voucher_code": "add_voucher_code", "code": "foo"}
        )
        ...
        assert resp.context_data["voucher_add_error"] == ["Voucher has expired"]
        assert resp.context_data["total_cost"] == 30
        ...
        paid = baker.make(
            Booking,
            user=self.user,
            event__event_type="regular_session",
            paid=True,
            voucher=voucher,
        )
        resp = self.client.post(
        ...
        baker.make(
            Booking, event=self.workshop, user=self.user, voucher=voucher, paid=True
        )
        resp = self.client.post(
            self.url,
            data={"add_voucher_code": "add_voucher_code", "code": voucher.code},
        ...
        voucher.save()
        baker.make(
            Booking, event=self.private, user=self.user, voucher=voucher, paid=False
        )
    ...
    def test_total_voucher_validation(self):
        voucher = baker.make(
            TotalVoucher, code="test_amount", discount_amount=10, activated=False
        )
        baker.make_recipe(
            "booking.membership",
            membership_type=self.membership_type,
        ...
        baker.make(
            ...
            total_voucher_code=voucher.code,
            paid=True,
        )
        resp = self.client.post(
            self.url,
    ...
    def test_total_voucher_greater_than_checkout_amount(self):
        voucher = baker.make(
            TotalVoucher, code="test_amount", discount_amount=5, activated=True
        )
        ...
        resp = self.client.post(
            self.url,
            data={"add_voucher_code": "add_voucher_code", "code": "test_amount"},
        )
    ...
    def test_apply_voucher_to_multiple_bookings(self):
        ...
        resp = self.client.post(
            self.url,
            data={"refresh_voucher_code": "refresh_voucher_code", "code": "test"},
        )
    ...
    def test_apply_multiple_vouchers_to_different_items(self):
        ...
        assert sorted(
        ...
        ) == [
            {
                "booking": regular_booking,
                "original_cost": 10,
                "voucher_applied": {"code": "foo", "discounted_cost": 9},
            },
            {
                "booking": private_booking,
                "original_cost": 30,
        ...
        assert sorted(resp.context_data["applied_voucher_codes_and_discount"]) == [
            ("foo", 10, None),
            ("test", 50, None),
        ]
...
class GuestShoppingBasketTests(TestCase):
    ...
    def test_with_invalid_vouchers(self):
        ...
        gift_voucher2 = baker.make_recipe("booking.gift_voucher_11")
        session.update(
            {"purchases": {"gift_vouchers": [gift_voucher1.id, gift_voucher2.id, 999]}}
        )
        session.save()
        resp = self.client.get(self.url)
        assert resp.context_data["unpaid_items"]
