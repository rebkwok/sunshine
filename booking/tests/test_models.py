...
def test_can_cancel_with_daylight_savings_time(
    mock_tz, now, event_date, can_cancel, cancellation_period
):
    mock_tz.now.return_value = now
    event = baker.make_recipe(
        "booking.future_EV", date=event_date, cancellation_period=cancellation_period
    )
...
class BookingTests(TestCase):
    ...
    def test_event_spaces_left_does_not_count_cancelled_or_no_shows(self):
        ...
        for user in self.users:
            baker.make_recipe("booking.booking", user=user, event=self.event)
        baker.make_recipe("booking.booking", event=self.event, no_show=True)
        baker.make_recipe("booking.booking", event=self.event, status="CANCELLED")
    ...
    def test_booking_full_event(self):
        ...
        baker.make_recipe("booking.booking", event=self.event_with_cost, _quantity=3)
        # we need to get the event again as spaces_left is cached property
        event = Event.objects.get(id=self.event_with_cost.id)
        with self.assertRaises(ValidationError):
            Booking.objects.create(event=event, user=self.users[0])
    ...
    def test_can_create_cancelled_booking_for_full_event(self):
        ...
        self.assertEqual(Booking.objects.filter(event=self.event_with_cost).count(), 4)
    ...
    def test_reopening_booking_sets_date_reopened(self, mock_tz):
        ...
        user = self.users[0]
        booking = baker.make_recipe(
            "booking.booking", event=self.event_with_cost, user=user, status="CANCELLED"
        )
...
def test_membership(membership_type):
    ...
    past_membership = baker.make(
        Membership, membership_type=membership_type, month=2, year=2022
    )
    current_membership = baker.make(
        Membership, membership_type=membership_type, month=now.month, year=now.year
    )
    future_membership = baker.make(
        Membership,
...
def test_gift_voucher_properties(gift_voucher_types):
    ...
    assert gift_voucher.start_date == start_of_day_in_local_time(datetime.today()).strftime("%d-%b-%Y")
...
def test_create_gift_voucher_from_existing_total_voucher(gift_voucher_types):
    ...
    gift_voucher = baker.make(
        GiftVoucher,
        gift_voucher_type=gift_voucher_types["total"],
...
def test_create_gift_voucher_from_existing_event_type_voucher(gift_voucher_types):
    ...
    gift_voucher = baker.make(
        GiftVoucher,
        gift_voucher_type=gift_voucher_types["private"],
    ...
    voucher = baker.make(
        ItemVoucher,
        event_types=["private", "regular_session"],
...
@pytest.mark.django_db
def test_create_gift_voucher_from_existing_membership_voucher(
    gift_voucher_types, membership_type
):
    # voucher matches gift voucher type
    ...
    gift_voucher = baker.make(
        GiftVoucher,
        gift_voucher_type=gift_voucher_types["membership_2"],
        item_voucher=voucher,
    )
    assert gift_voucher.voucher.id == voucher.id
    voucher.refresh_from_db()
    ...
    gift_voucher = baker.make(
        GiftVoucher,
        gift_voucher_type=gift_voucher_types["membership_4"],
        item_voucher=voucher,
    )
    assert gift_voucher.voucher.id != voucher.id
    assert ItemVoucher.objects.count() == 2
...
def test_gift_voucher_str(gift_voucher_types):
    ...
    assert (
        str(gift_voucher)
        == f"{gift_voucher.voucher.code} - Gift Voucher: Private Lesson - foo@bar.com"
    )
    ...
    assert (
        str(gift_voucher1)
        == f"{gift_voucher1.voucher.code} - Gift Voucher: £10 - foo@bar.com"
    )
...
@pytest.mark.django_db
def test_gift_voucher_cost(gift_voucher_types):
    gvt = gift_voucher_types["total"]
