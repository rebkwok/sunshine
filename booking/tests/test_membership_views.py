...
def test_user_memberships_get(client, configured_user, membership_type):
    ...
    expired = baker.make_recipe(
        "booking.membership", user=configured_user, paid=True, month=last_month.month, year=last_month.year
    )
    full = baker.make_recipe(
        "booking.membership", membership_type=membership_type, user=configured_user, 
        paid=True
    )
    ...
    assert [
        mem.id for mem in resp.context_data["memberships"]
    ] == [paid.id]
    ...
    assert sorted([
        mem.id for mem in resp.context_data["memberships"]
    ]) == [paid.id, expired.id, full.id]

    
# MembershipDetailView (views/membership_views.py)
def test_detail_url(client, configured_user):
    membership = baker.make_recipe("booking.membership", user=configured_user, paid=True)
...
def test_membership_purchase_options(freezer, client, configured_user, membership_type):
    ...
    baker.make(RegularClass, date=datetime(2020, 10, 25, 10, 0, tzinfo=timezone.utc))
    baker.make(RegularClass, date=datetime(2020, 11, 1, 10, 0, tzinfo=timezone.utc))
    resp = client.get(buy_url)
    ...
    future_class = baker.make(RegularClass, date=datetime(2020, 10, 30, 10, 0, tzinfo=timezone.utc))
    resp = client.get(buy_url)
    ...
    future_class.date=datetime(2020, 12, 30, 10, 0, tzinfo=timezone.utc)
    future_class.save()
    client.force_login(configured_user)
    # time is beginning of month, shows next months options as well
    resp = client.get(buy_url)
...
def test_membership_purchase_options_with_unpaid_items(
    ...
    # make a future class for this month
    baker.make(RegularClass, date=datetime(2020, 10, 30, 10, 0, tzinfo=timezone.utc))
    resp = client.get(buy_url)
    assert resp.context_data["options"] == [
        {
            "membership_type": membership_type, 
    ...
    baker.make(
        Membership, membership_type=membership_type, user=configured_user, 
        paid=False, month=10, year=2020
    )
    baker.make(
        Membership, membership_type=membership_type, user=configured_user, 
        paid=True, month=10, year=2020
    )
    baker.make(
        Membership, membership_type=membership_type, user=configured_user, 
        paid=False, month=11, year=2020
    )
    resp = client.get(buy_url)
    assert resp.context_data["options"] == [
        {
...
def test_ajax_add_membership(client, configured_user, membership_type):
    ...
    resp = client.post(
        add_url, 
        {"membership_type_id": membership_type.id, "month": 3, "year": 2022}
    )
    assert resp.status_code == 200
