...
class BookingListViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.regular_sessions_bookings = [
            ...
            for event in self.regular_sessions
        ]
        self.event_bookings = [
            baker.make_recipe("booking.booking", user=self.user, event=event)
    ...
    def test_cancelled_booking_shown_in_booking_list(self):
        ...
        baker.make_recipe(
            "booking.booking", user=self.user, event=ev, status="CANCELLED"
        )
    ...
    def test_buttons_disabled_if_user_has_outstanding_fees(self):
        ...
        baker.make_recipe(
            "booking.booking", event=full_event, user=self.user, status="CANCELLED"
        )
...
class BookingHistoryListViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.past_booking = baker.make_recipe("booking.past_booking", user=cls.user)
