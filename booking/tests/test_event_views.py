...
class EventListViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.reg_class1 = baker.make_recipe(
            "booking.future_PC", name="Class 1", venue=venue
        )
        cls.reg_class2 = baker.make_recipe(
            "booking.future_PC", name="Class 2", venue=venue
        )
    ...
    def test_event_list_with_name_day_and_time(self, mock_now):
        ...
        resp = self.client.get(url)
        assert len(resp.context_data["events"]) == 2
        assert [ev.id for ev in resp.context_data["events"]] == [
            self.reg_class1.id,
            reg_class3.id,
        ]
    ...
    def test_outstanding_fees_shows_banner(self):
        baker.make_recipe(
            "booking.booking",
            user=self.user,
            event=self.reg_class1,
            status="CANCELLED",
            cancellation_fee_incurred=True,
        )
        resp = self.client.get(self.classes_url)
        assert (
            "Your account is locked for booking due to outstanding fees"
            in resp.rendered_content
        )
    ...
    def test_buttons_disabled_if_user_has_outstanding_fees(self):
        ...
        resp = self.client.get(self.classes_url)
        assert (
            'id="book_button_disabled"' in resp.rendered_content
...
class EventDetailViewTests(TestCase):
    ...
    def test_event_full(self):
        baker.make_recipe(
            "booking.booking", event=self.event, _quantity=self.event.max_participants
        )
        resp = self.client.get(self.url())
    ...
    def test_cancellation_information_displayed_cancellation_not_allowed(self):
        ...
        assert (
            "Cancellation is allowed up to 12 hours prior to the workshop "
