    ...
    def callable():
        NAME_CHOICES = list(
            set(
                ...
                .filter(event_type=event_type, date__gte=timezone.now())
                .order_by("name")
...
class EventsFilter(forms.Form):
    name = forms.ChoiceField(
        choices=(0, 0), widget=forms.Select(attrs={"class": "form-control input-xs"})
    )
...
class GiftVoucherForm(forms.ModelForm):
    ...
    def __init__(self, **kwargs):
        ...
        self.fields["gift_voucher_type"].queryset = GiftVoucherType.objects.filter(
            active=True
        )
        ...
        if self.instance.id:
            ...
            submit_button = (
                Submit(
                    "submit",
                    "Add to cart",
                    css_class="btn btn-sunshine tra-sunshine-hover",
                )
                if user is not None
                else Submit(
                    "submit",
