    ...
    def __init__(self, *args, **kwargs):
        ...
        if DataPrivacyPolicy.current():
            ...
            self.fields["content"] = forms.CharField(
                initial=self.data_privacy_policy.content, required=False
            )
    ...
    def signup(self, request, user):
        ...
        if hasattr(self, "data_privacy_policy"):
            SignedDataPrivacy.objects.create(
                user=user,
                version=self.data_privacy_policy.version,
...
class DisclaimerForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.id:
            self.disclaimer_content = DisclaimerContent.objects.get(
                version=self.instance.version
            )
        else:
            self.disclaimer_content = DisclaimerContent.current()

        self.fields["health_questionnaire_responses"].required = any(
            field.get("required") for field in self.disclaimer_content.form
        )
        ...
        self.fields["emergency_contact_phone"].validators = [
            account_validators.phone_number_validator
        ]
        ...
        if self.instance.id:
            buttons.append(
                HTML(
                    f'<a class="btn  btn-xs btn-color-01 tra-01-hover" href="{back_url}">Cancel</a>'
                )
            )
        self.helper.layout = Layout(
            ...
            HTML(
                '<p>If you have answered "Yes" to any of the above questions, please consult your doctor before '
...
class DisclaimerContactUpdateForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            HTML(
                f'<a class="btn btn-color-01 tra-01-hover" href="{back_url}">Cancel</a>'
            ),
        )
