...
class PolicyAdminFormMixin:
    ...
    def clean(self):
        ...
        if current_policy and current_policy.content == new_content:
            self.add_error(
                None,
                "No changes made from previous version; "
...
class DisclaimerContentAdminForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.id or self.instance.is_draft:
            self.fields["version"].required = False
            self.fields["disclaimer_terms"].widget.attrs = {
                "style": "width: 100%;",
                "rows": 20,
            }
            if not self.instance.id:
...
class DisclaimerContentAdmin(admin.ModelAdmin):
    ...
    def note(self, obj):
        return (
            "THIS DISCLAIMER CONTENT IS PUBLISHED AND CANNOT BE EDITED. TO MAKE CHANGES, "
...
class OnlineDisclaimerAdmin(admin.ModelAdmin):
    ...
    def health_questionnaire(self, obj):
        return format_questionnaire_responses_to_html(
            obj.health_questionnaire_responses
        )
...
class UserAdmin(BaseUserAdmin):
    ...
    def disclaimer_link(self, obj):
        if has_active_disclaimer(obj) or has_expired_disclaimer(obj):
            ...
            return format_html(
                "<a href={}><img src='/static/admin/img/icon-viewlink.svg' alt='View'></a>",
                url,
            )
