...
from ..admin import (
    ...
    DataPrivacyPolicyAdminForm,
    DisclaimerContentAdminForm,
)
...
class SignUpFormTests(TestCase):
    ...
    def test_get_signup_view_with_username(self):
        ...
        self.assertEqual(resp.context_data["form"].fields["username"].initial, "test")

    def test_signup_form_with_invalid_data(self):
        # first_name must have 30 characters or fewer
        self.form_data.update(
            {"first_name": "abcdefghijklmnopqrstuvwxyz12345", "last_name": "User"}
        )
...
class CookiePolicyAdminFormTests(TestCase):
    ...
    def test_validation_error_if_no_changes(self):
        ...
        form = CookiePolicyAdminForm(
            data={"content": "Foo", "version": 1.5, "issue_date": policy.issue_date}
        )
        self.assertFalse(form.is_valid())
        self.assertEqual(
...
def test_disclaimer_content_admin_form_new_with_previous_version():
    ...
    assert (
        form.fields["version"].help_text
        == "Current version is 1.0.  Leave blank for next major version (2.0)"
    )
...
def test_disclaimer_content_admin_form_bad_version():
    ...
    assert form.errors == {
        "version": [
            "New version must increment current version (must be greater than 1.0)"
        ]
    }
