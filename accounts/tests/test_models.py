...
def test_cannot_change_existing_published_disclaimer_version():
    content = make_disclaimer_content(
        disclaimer_terms="first version", version=4, is_draft=True
    )
    content.version = 3.8
    content.save()
...
def test_disclaimer_content_str():
    disclaimer_content = make_disclaimer_content()
    assert (
        str(disclaimer_content)
        == f"Disclaimer Terms & PARQ - Version {disclaimer_content.version} (published)"
    )
...
def test_archived_disclaimer_str(user):
    ...
    assert (
        str(disclaimer)
        == "Test User - V5.0 - 01 Jul 2019, 19:00 (archived 20 Jan 2020, 18:00)"
    )
...
def test_delete_online_disclaimer(user):
    ...
    ArchivedDisclaimer.objects.all().delete()
    cache.clear()
