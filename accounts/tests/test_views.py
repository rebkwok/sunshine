...
def test_profile_update_view_post(client):
    ...
    url = reverse('accounts:update_profile')
    client.force_login(user)
    client.post(url, {'username': user.username, 'first_name': 'Fred', 'last_name': user.last_name})
...
def test_disclaimer_create_view_shows_msg_if_already_has_disclaimer(client, user, disclaimer_content):
...
def test_disclaimer_create_view_submitting_form_without_valid_password(client, user, disclaimer_content):
    client.force_login(user)
...
def test_disclaimer_create_view_terms_accepted(client, user, disclaimer_content):
    ...
    assert OnlineDisclaimer.objects.count() == 0
    url = reverse('accounts:disclaimer_form', args=(user.id,))
    resp = client.post(url, {**get_disclaimer_form_data(user), "date_of_birth": '01-Jan-2020'})
    form = resp.context_data["form"]
    assert form.errors == {"date_of_birth": ["You must be at least 16 years old to register and book classes"]}


def test_disclaimer_create_view_phone_number_validation(client, user, disclaimer_content):
    client.force_login(user)
    assert OnlineDisclaimer.objects.count() == 0
    url = reverse('accounts:disclaimer_form', args=(user.id,))
...
def test_disclaimer_create_view_submitting_form_creates_disclaimer(client, user, disclaimer_content):
    client.force_login(user)
    assert OnlineDisclaimer.objects.count() == 0
    url = reverse('accounts:disclaimer_form', args=(user.id,))
...
def test_disclaimer_create_view_updating_disclaimer_health_questionnaire(client, user):
    ...
    make_online_disclaimer(
        user=user, version=content_with_questionnaire.version,
    ...
    make_online_disclaimer(
        user=user1, version=content_with_questionnaire.version,
        date=timezone.now() - timedelta(days=370),
        health_questionnaire_responses={
            "Say something": "Boo",
...
def test_disclaimer_update_view_only_uses_active_disclaimer(client, user, disclaimer_content):
    ...
    # expired
    make_online_disclaimer(
        user=user, version=disclaimer_content.version,
        date=timezone.now() - timedelta(days=370),
    )
    client.force_login(user)
    url = reverse('accounts:disclaimer_form_update', args=(user.id,))
