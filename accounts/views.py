...
def profile(request):
    ...
    latest_disclaimer = (
        request.user.online_disclaimer.exists()
        and request.user.online_disclaimer.latest("id")
    )
...
class SignedDataPrivacyCreateView(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        update_needed = SignedDataPrivacy.objects.filter(
            user=self.request.user, version__lt=DataPrivacyPolicy.current_version()
        ).exists() and not has_active_data_privacy_agreement(self.request.user)
...
def data_privacy_policy(request):
    return render(
        request,
        "accounts/data_privacy_policy.html",
        {
...
class DisclaimerContactUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not has_active_disclaimer(self.disclaimer_user):
            return HttpResponseRedirect(
                reverse("accounts:disclaimer_form", args=(self.disclaimer_user.id,))
            )
...
class DisclaimerCreateView(LoginRequiredMixin, DisclaimerFormMixin, CreateView):
    ...
    def get_form(self, *args, **kwargs):
        ...
        if has_expired_disclaimer(self.disclaimer_user):
            updating_disclaimer = OnlineDisclaimer.objects.filter(
                user=self.disclaimer_user
            ).last()
        else:
        ...
        for field in form.fields["health_questionnaire_responses"].fields:
            ...
            if (
                ...
                if hasattr(field.widget, "choices") and isinstance(
                    previous_response, list
                ):
                    if (
                        set(previous_response)
                        - {choice[0] for choice in field.widget.choices}
                        == set()
                    ):
