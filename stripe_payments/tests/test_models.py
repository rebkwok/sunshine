...
def test_invoice_str():
    invoice = baker.make(
        Invoice, username="test@test.com", invoice_id="foo123", amount="10"
    )
...
def test_invoice_item_count():
    invoice = baker.make(
        ...
        gift_vouchers=baker.make(
            GiftVoucher, gift_voucher_type__discount_amount=10, _quantity=1
        ),
    )
...
def test_invoice_items_metadata():
    ...
    membership = baker.make(
        Membership,
        membership_type__cost=10,
        membership_type__name="test membership",
        invoice=invoice,
    )
    booking = baker.make(
    ...
    gift_voucher = baker.make(
        GiftVoucher, gift_voucher_type__discount_amount=10, invoice=invoice
    )
...
def test_invoice_items_summary():
    ...
    gift_voucher = baker.make(
        GiftVoucher,
        gift_voucher_type__event_type="private",
        gift_voucher_type__override_cost=Decimal(10),
        invoice=invoice,
    )
...
def test_create_stripe_payment_intent_instance_from_pi(get_mock_payment_intent):
    ...
    pi, created = StripePaymentIntent.update_or_create_payment_intent_instance(
    ...
    seller = baker.make(Seller, user__email="testuser@test.com")
    pi, created = StripePaymentIntent.update_or_create_payment_intent_instance(
        payment_intent, invoice, seller=seller
    )
    assert not created
    assert StripePaymentIntent.objects.count() == 1
