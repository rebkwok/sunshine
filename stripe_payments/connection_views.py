...
def connect_stripe_view(request):
    ...
    return render(
        request, "stripe_payments/connect_stripe.html", {"site_seller": site_seller}
    )
    ...
    def get(self, request):
        ...
        params = {
            ...
            "redirect_uri": request.build_absolute_uri(
                reverse("stripe_payments:authorize_stripe_callback")
            ),
        }
...
class StripeAuthorizeCallbackView(View):
    def get(self, request):
        ...
        if code:
            ...
            seller.save()
            logger.info("Stripe account connected: %s", seller.stripe_user_id)
            ActivityLog.objects.create(
                log=f"Stripe account connected: {seller.stripe_user_id}"
            )
