...
def get_session_types():
    def callable():
        SESSION_TYPE_CHOICES = list(
            TimetableSession.objects.select_related("venue", "session_type")
            .distinct()
            .order_by("session_type__order")
...
class UploadTimetableForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["show_on_site"] = forms.BooleanField(
            initial=True,
            help_text="(Classes will be available immediately for booking)",
            required=False,
        )
        ...
        self.fields["sessions"] = forms.ModelMultipleChoiceField(
            widget=forms.CheckboxSelectMultiple(attrs={"class": "select-checkbox"}),
    ...
    def clean(self):
        ...
        if start_date:
            ...
            try:
                ...
                if start_date >= timezone.now().date():
                    ...
                    self.add_error("start_date", "Must be in the future")
            except ValueError:
                self.add_error(
                    "start_date",
                    "Invalid date format.  Select from "
        ...
        if not self.errors.get("end_date") and not self.errors.get("start_date"):
            if end_date >= start_date:
                ...
                self.add_error("end_date", "Cannot be before start date")
        return cleaned_data
