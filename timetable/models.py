    ...
    def clean(self):
        if self.display_on_site and not self.description:
            raise ValidationError(
                "To display this session type on the 'what we offer' page, you also need to add a description"
            )
...
class Venue(models.Model):
    ...
    def distinct_locations_in_order(cls):
        locations_with_order = cls.objects.distinct("order", "location").values_list(
...
class TimetableSession(models.Model):
    ...
    def active_locations(cls):
        ...
        active_locations = set(
            cls.objects.filter(show_on_timetable_page=True).values_list(
                "venue__location__name", flat=True
            )
        )
