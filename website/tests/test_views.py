...
def test_new_home_page(client):
    ...
    baker.make(TeamMember, photo=None, _quantity=2)
    testimonials = baker.make(Testimonial, _quantity=2)
    response = client.get(reverse("website:home") + "?new-home-page")
    assert {t.id for t in response.context["testimonials"]} == {
        t.id for t in testimonials
    }
...
def test_session_types(client):
    ...
    assert [st.id for st in resp.context["session_types"]] == [
        session_type2.id,
        session_type1.id,
    ]
