...
class ContactForm(forms.Form):
    ...
    cc = forms.BooleanField(
        widget=forms.CheckboxInput(attrs={"class": "regular-checkbox", "id": "cc_id"}),
    ...
    message = forms.CharField(
        widget=forms.Textarea(
            attrs={"class": "form-control email-message", "rows": 10}
        ),
        label="Message",
        required=True,
    )
    ...
    data_privacy_accepted = forms.BooleanField(
        widget=forms.CheckboxInput(
            attrs={"class": "regular-checkbox", "id": "data_privacy_accepted_id"}
        ),
        label=mark_safe(
