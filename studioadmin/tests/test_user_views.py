...
class UserListViewTests(TestPermissionMixin, TestCase):
    ...
    def test_all_users_are_displayed(self):
        ...
        self.client.login(username=self.staff_user.username, password="test")
        resp = self.client.get(self.url)
        self.assertEqual(list(resp.context_data["users"]), list(User.objects.all()))

    def test_abbreviations_for_long_username(self):
        """
        Usernames > 15 characters are split to 2 lines
        """
        baker.make_recipe("booking.user", username="test123456789101112")
        self.client.login(username=self.staff_user.username, password="test")
        resp = self.client.get(self.url)
    ...
    def test_abbreviations_for_long_names(self):
        ...
        self.client.login(username=self.staff_user.username, password="test")
        resp = self.client.get(self.url)
        self.assertIn("namewith-</br>morethan12characters", resp.rendered_content)
        self.assertIn("name-</br>with-three-hyphens", resp.rendered_content)

    def test_user_filter(self):
        baker.make_recipe(
            "booking.user", username="FooBar", first_name="AUser", last_name="Bar"
        )
        baker.make_recipe(
            "booking.user", username="Testing1", first_name="aUser", last_name="Bar"
        )
        baker.make_recipe(
            "booking.user", username="Testing2", first_name="BUser", last_name="Bar"
        )
        self.client.login(username=self.staff_user.username, password="test")
...
def test_user_disclaimer_view_questionnaire_responses(client, superuser):
    ...
    resp = client.get(reverse("studioadmin:user_disclaimer", args=(user.id,)))
    assert (
