...
class WaitingListViewStudioAdminTests(TestPermissionMixin, TestCase):
    ...
    def test_can_access_as_staff_user(self):
        ...
        resp = self.client.get(
            reverse(self.url_string, kwargs={"event_slug": event.slug})
        )
    ...
    def test_waiting_list_users_shown(self):
        ...
        baker.make_recipe("booking.waiting_list_user", event=event1, _quantity=3)
        resp = self.client.get(
            reverse(self.url_string, kwargs={"event_slug": event.slug})
        )
    ...
    def test_remove_waiting_list_users(self):
        ...
        resp = self.client.post(
            reverse(self.url_string, kwargs={"event_slug": event.slug}),
            data={"remove_user": [event_wl[0].id]},
        )
