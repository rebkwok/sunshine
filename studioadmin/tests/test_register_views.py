...
class EventRegisterListViewTests(TestPermissionMixin, TestCase):
    ...
    def test_event_register_list_shows_todays_events(self):
        ...
        baker.make_recipe("booking.past_class", _quantity=4)
        baker.make_recipe(
            "booking.past_class", date=timezone.now().replace(hour=0, minute=1)
        )
        resp = self.client.get(self.url)
    ...
    def test_event_register_list_shows_event_in_next_week_by_default(self):
        baker.make_recipe(
            "booking.future_PC", date=timezone.now() + timedelta(6), _quantity=4
        )
...
class RegisterViewTests(TestPermissionMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.ev = baker.make_recipe(
            "booking.future_EV", max_participants=3, cancellation_fee=1.00
        )
        cls.pc_url = reverse("studioadmin:event_register", args=(cls.pc.slug,))
        cls.pc_no_max_url = reverse(
            "studioadmin:event_register", args=(cls.pc_no_max.slug,)
        )
    ...
    def test_register_shows_event_bookings(self):
        bookings = baker.make_recipe(
            "booking.booking", status="OPEN", event=self.pc, _quantity=2
        )
    ...
    def test_cancelled_bookings_not_shown(self):
        ...
        baker.make_recipe(
            "booking.booking", status="CANCELLED", event=self.pc, _quantity=2
        )
        resp = self.client.get(self.pc_url)
        self.assertEqual(
            sorted([booking.id for booking in resp.context_data["bookings"]]),
    ...
    def test_no_show_bookings_shown(self):
        ...
        no_show_bookings = baker.make_recipe(
            "booking.booking", status="OPEN", no_show=True, event=self.pc, _quantity=1
        )
        resp = self.client.get(self.pc_url)
        self.assertEqual(
            sorted([booking.id for booking in resp.context_data["bookings"]]),
...
class RegisterAjaxAddBookingViewsTests(TestPermissionMixin, TestCase):
    ...
    def test_reopen_cancelled_booking(self):
        booking = baker.make_recipe(
            "booking.booking", user=self.user, event=self.pc, status="CANCELLED"
        )
    ...
    def test_user_choices(self):
        ...
        # open booking
        baker.make_recipe(
            "booking.booking", user=self.user, event=self.pc, status="OPEN"
        )
        # no_show_booking
        baker.make_recipe(
            "booking.booking", user=user, event=self.pc, status="OPEN", no_show=True
        )
        # cancelled_booking
        baker.make_recipe(
            "booking.booking", user=user1, event=self.pc, status="CANCELLED"
        )
    ...
    def test_already_open_booking(self, mock_messages):
        ...
        mock_messages.assert_called_once_with(
            request, "Open booking for this user already exists"
        )
...
class RegisterAjaxDisplayUpdateTests(TestPermissionMixin, TestCase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.pc = baker.make_recipe("booking.future_PC", max_participants=3)
        cls.pc_with_fee = baker.make_recipe(
            "booking.future_PC", max_participants=3, cancellation_fee=1.00
        )
    ...
    def setUp(self):
        ...
        self.booking = baker.make_recipe(
            "booking.booking", user=self.user, event=self.pc
        )
        self.toggle_attended_url = reverse(
            "studioadmin:ajax_toggle_attended", args=(self.booking.id,)
        )
    ...
    def test_ajax_toggle_no_show_no_waiting_list_email_for_full_event_within_30_mins(
        self,
    ):
        baker.make_recipe("booking.booking", event=self.pc, _quantity=2)
        pc = Event.objects.get(id=self.pc.id)
    ...
    def test_ajax_toggle_attended_cancelled_booking_full_event(self):
        ...
        self.assertFalse(resp.json()["attended"])
        self.assertEqual(
            resp.json()["alert_msg"], "Class is now full, cannot reopen booking."
        )
    ...
    def test_ajax_toggle_attended_cancellation_fee(self):
        booking_with_fee_unpaid = baker.make_recipe(
            "booking.booking", event=self.pc_with_fee, cancellation_fee_incurred=True
        )
        booking_with_fee_paid = baker.make_recipe(
            "booking.booking",
            event=self.pc_with_fee,
            cancellation_fee_incurred=True,
            cancellation_fee_paid=True,
        )

        url = reverse(
            "studioadmin:ajax_toggle_attended", args=(booking_with_fee_unpaid.id,)
        )
        ...
        url = reverse(
            "studioadmin:ajax_toggle_attended", args=(booking_with_fee_paid.id,)
        )
        resp = self.client.post(url, {"attendance": "attended"}).json()
        assert resp["attended"]
