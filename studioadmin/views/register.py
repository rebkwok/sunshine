    ...
    def get_queryset(self):
        ...
        if show_all:
            return Event.objects.filter(
                event_type=event_type, date__gte=today
            ).order_by("date")
        else:
            end_date = timezone.now() + timedelta(7)
            return Event.objects.filter(
                event_type=event_type, date__gte=today, date__lte=end_date
            ).order_by("date")
...
def ajax_toggle_attended(request, booking_id):
    ...
    if attendance == "attended":
        if (
            booking.no_show or booking.status == "CANCELLED"
        ) and booking.event.spaces_left == 0:
            ev_type = (
    ...
    if (
        event_was_full
        and attendance == "no-show"
        and booking.event.date > (timezone.now() + timedelta(minutes=30))
    ):
