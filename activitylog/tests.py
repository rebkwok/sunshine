...
def test_activity_log_admin_timestamp_display():
    ...
    assert log_admin.timestamp_formatted(log_query) == log_query.timestamp.strftime(
        "%d-%b-%Y %H:%M:%S (%Z)"
    )
...
class DeleteOldActivityLogsTests(TestCase):
    def setUp(self):
        ...
        self.log_11monthsold = baker.make(
            ActivityLog,
            log="message",
            timestamp=self.mock_now - relativedelta(months=11),
        )
        self.log_25monthsold = baker.make(
            ActivityLog,
            log="message",
            timestamp=self.mock_now - relativedelta(months=25),
        )
        self.log_37monthsold = baker.make(
            ActivityLog,
            log="message",
            timestamp=self.mock_now - relativedelta(months=37),
        )
            ...
            with override_settings(LOG_FOLDER=tmpdir):
                ...
                mock_run.assert_called_once_with(
                    [
                        ...
                        "cp",
                        str(local_filepath),
                        os.path.join(settings.S3_LOG_BACKUP_PATH, filename),
                    ],
                    check=True,
                )
